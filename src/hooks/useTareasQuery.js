...
export const useTareasQuery = (token, proyectoId = null) => {
  ...
  const crearTarea = useMutation({
    ...
    onSuccess: () => { qc.invalidateQueries(['tareas']); toast.success('Tarea creada correctamente'); },
    onError: (err) => toast.error(isGuest ? 'Crea una cuenta gratis para crear tareas.' : (err.message || 'Error al crear la tarea')),
  });
  ...
  const cambiarEstado = useMutation({
    ...
    onSuccess: () => { qc.invalidateQueries(['tareas']); toast.success('Estado actualizado correctamente'); },
    onError: (err) => toast.error(isGuest ? 'Crea una cuenta gratis para editar tareas.' : (err.message || 'Error al actualizar el estado')),
  });
  ...
  const eliminarTarea = useMutation({
    ...
    onSuccess: () => { qc.invalidateQueries(['tareas']); toast.success('Tarea eliminada correctamente'); },
    onError: (err) => toast.error(isGuest ? 'Crea una cuenta gratis para eliminar tareas.' : (err.message || 'Error al eliminar la tarea')),
  });
  ...
  const duplicarTarea = useMutation({
    ...
    onSuccess: () => { qc.invalidateQueries(['tareas']); toast.success('Tarea duplicada correctamente'); },
    onError: (err) => toast.error(isGuest ? 'Crea una cuenta gratis para duplicar tareas.' : (err.message || 'Error al duplicar la tarea')),
  });
  ...
  const actualizarFechaExpiracion = useMutation({
    mutationFn: async ({ id, expiration_date }) => {
      ...
      if (!dateRegex.test(expiration_date)) throw new Error('Formato de fecha inválido. Debe ser YYYY-MM-DDTHH:mm');
      return http.put(`/api/v1/tareas/${id}`, { expiration_date });
    },
    onSuccess: () => { qc.invalidateQueries(['tareas']); toast.success('Fecha de expiración actualizada correctamente'); },
    onError: (err) => toast.error(isGuest ? 'Crea una cuenta gratis para editar tareas.' : (err.message || 'Error al actualizar la fecha de expiración')),
  });
