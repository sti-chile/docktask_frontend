...
export const useWorkspaceQuery = (token) => {
  ...
  const crearWorkspace = useMutation({
    ...
    onSuccess: () => { qc.invalidateQueries(['workspaces']); toast.success('Workspace creado correctamente'); },
    onError: (err) => toast.error(isGuest ? 'Crea una cuenta gratis para crear espacios.' : (err.message || 'Error al crear el workspace')),
  });
  ...
  const actualizarWorkspace = useMutation({
    ...
    onSuccess: () => { qc.invalidateQueries(['workspaces']); toast.success('Workspace actualizado correctamente'); },
    onError: (err) => toast.error(isGuest ? 'Crea una cuenta gratis para editar espacios.' : (err.message || 'Error al actualizar el workspace')),
  });
  ...
  const eliminarWorkspace = useMutation({
    ...
    onSuccess: () => { qc.invalidateQueries(['workspaces']); toast.success('Workspace eliminado correctamente'); },
    onError: (err) => toast.error(isGuest ? 'Crea una cuenta gratis para eliminar espacios.' : (err.message || 'Error al eliminar el workspace')),
  });
