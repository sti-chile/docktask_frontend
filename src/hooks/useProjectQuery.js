...
export const useProjectQuery = (token, workspaceId = null) => {
  ...
  const crearProyecto = useMutation({
    ...
    onSuccess: () => { qc.invalidateQueries(['proyectos']); toast.success('Proyecto creado correctamente'); },
    onError: (err) => toast.error(isGuest ? 'Crea una cuenta gratis para crear proyectos.' : (err.message || 'Error al crear el proyecto')),
  });
  ...
  const actualizarProyecto = useMutation({
    ...
    onSuccess: () => { qc.invalidateQueries(['proyectos']); toast.success('Proyecto actualizado correctamente'); },
    onError: (err) => toast.error(isGuest ? 'Crea una cuenta gratis para editar proyectos.' : (err.message || 'Error al actualizar el proyecto')),
  });
  ...
  const eliminarProyecto = useMutation({
    ...
    onSuccess: () => { qc.invalidateQueries(['proyectos']); toast.success('Proyecto eliminado correctamente'); },
    onError: (err) => toast.error(isGuest ? 'Crea una cuenta gratis para eliminar proyectos.' : (err.message || 'Error al eliminar el proyecto')),
  });
