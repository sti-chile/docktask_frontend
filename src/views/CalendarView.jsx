...
const MiniCard = ({ event }) => (
  <div className="bg-white border rounded shadow p-1 text-xs overflow-hidden">
    ...
    <div className="text-gray-500 truncate">{event.tarea?.descripcion || event.tarea?.mensaje || ''}</div>
  </div>
);
...
const CalendarView = ({ tareas, mensajes }) => {
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      <h1 className="text-2xl font-bold mb-6">Calendario de Tareas (por fecha de expiración)</h1>
      <Calendar
        localizer={localizer}
        events={events}
        startAccessor="start"
