...
const CycleDetail = ({ token, workspaceId, cycleId }) => {
    ...
    const handleComplete = async () => {
        if (
            !window.confirm("¿Completar este ciclo? Las tareas no completadas quedarán en backlog.")
        )
            return
    ...
    const completedTasks = tasks.filter(
        (t) => t.estado === "completada" || t.estado === "Completada"
    ).length
    ...
    return (
        <div className="space-y-6 max-w-4xl mx-auto">
            ...
            {/* Tasks list */}
            <div className="bg-white rounded-xl border border-gray-200 p-6">
                <h2 className="text-lg font-semibold text-gray-800 mb-4">Tareas · {totalTasks}</h2>
