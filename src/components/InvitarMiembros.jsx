...
const InvitarMiembros = ({ workspaceId, token }) => {
    ...
    useEffect(() => {
        let cancelled = false
        apiRef.current
            .getMiembrosWorkspace(workspaceId)
            .then((data) => {
                if (!cancelled) setMiembros(Array.isArray(data) ? data : [])
            })
            .catch(() => {
    ...
    return (
        <div className="space-y-4">
            <label className="block text-sm font-medium text-gray-700">Invitar miembros</label>
            ...
            <p className="text-xs text-gray-400">
                Escribí un nombre o usuario para buscar, o ingresá un correo y presioná Enter para
                invitar.
            </p>
