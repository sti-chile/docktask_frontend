      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {tracks.map(track => (
            <div
              key={track.id}
              className={`bg-white rounded-lg shadow border ${selectedTracks.includes(track.id) ? 'border-blue-500 ring-2 ring-blue-200' : 'border-gray-200'}`}
            >
              <div className="p-4">
                ...
                <div className="mt-4 grid grid-cols-2 gap-2 text-sm text-gray-500">
                  <div>
                    <span className="font-medium">Duración:</span> {formatDuration(track.duration)}
                  </div>
                  <div>
                    <span className="font-medium">Tamaño:</span> {formatFileSize(track.file_size)}
                  </div>
                  <div>
                    <span className="font-medium">Subido:</span> {new Date(track.created_at).toLocaleDateString()}
                  </div>
                  <div>
                    <span className="font-medium">Formato:</span> {track.mime_type.split('/')[1]}
                  </div>
                </div>
